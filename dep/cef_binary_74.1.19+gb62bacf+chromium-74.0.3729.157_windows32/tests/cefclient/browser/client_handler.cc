...
bool ClientHandler::OnProcessMessageReceived(
  ...
  // winsoft666: [update-cefclient] Handle custom message, such as 'mrtNotifyMsg'
  if (HandleProcessMessage(browser, message)) {
    return true;
  }
...
void ClientHandler::OnBeforeContextMenu(CefRefPtr<CefBrowser> browser,
  ...
  // winsoft666: [update-cefclient] Clear all context menu.
  model->Clear();
  return;
...
bool ClientHandler::OnContextMenuCommand(CefRefPtr<CefBrowser> browser,
  ...
  // winsoft666: [update-cefclient] Skip the test code of cefclient.
  return false;
